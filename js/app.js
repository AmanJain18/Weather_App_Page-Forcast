...
function formatDate(date) {
    ...
    currentDate.innerHTML = `${day}, ${dates} ${month}' ${year} ${hours}:${minutes}`;
}
...
function searchCity(event) {
    ...
    // axios API
    let apiKey = `6a6d6ec2575aa839225ac963ad952082`;
    ...
    axios.get(`${apiUrl}`).then(showSearchedCity).catch(err => {
        showingCity.innerHTML = `Waiting...`;
...
function onReload(event) {
    event.preventDefault();
    let defaultInputCity = "Mumbai";
    let showingCity = document.querySelector(`#show-city`);
    showingCity.innerHTML = defaultInputCity;

    // axios API
    let apiKey = `6a6d6ec2575aa839225ac963ad952082`;
    let mainLink = `https://api.openweathermap.org/data/2.5/weather?`;
    let units = `metric`;
    let cityName = defaultInputCity;
    // if (cityName === "") {
    //     return false;
    // }
...
function showPosition(position) {
    ...
    let apiKeyLocation = `6a6d6ec2575aa839225ac963ad952082`;
    let units = `metric`;
